...
type FeatureStatus = "Disabled" | "Testing" | "Enabled";
...
async function writeFeaturesAtomic(features: FeaturesFile): Promise<void> {
...
function dependencyStates(features: FeaturesFile, deps: string[] | undefined) {
  if (!deps?.length) return [];
  return deps.map((dep) => {
    const f = features[dep];
    return f
      ? { feature_name: dep, status: f.status }
      : { feature_name: dep, status: "UNKNOWN" as const, error: "FEATURE_NOT_FOUND" };
  });
}
...
function readErrorPayload(err: unknown, feature_name: string) {
  return {
...
const setFeatureStateSchema = z.object({
  ...
  state: z
    ...
    .describe(
      "Целевой status. Только три значения, регистрозависимо: 'Disabled' (выключено),"
    ...
    if (state === "Enabled" && feature.dependencies?.length) {
      ...
      if (blocking.length > 0) {
        return asJsonContent({
          ...
          message:
            `Cannot transition '${feature_name}' to Enabled: dependencies in Disabled `
...
server.tool(
  ...
  adjustTrafficRolloutSchema.shape,
  async (args) => {
    ...
    if (feature.status === "Disabled" && percentage > 0) {
      return asJsonContent({
        error: "LOCKED_BY_DISABLED_STATUS",
        message:
          `Cannot set traffic_percentage=${percentage} while '${feature_name}' is Disabled. `
